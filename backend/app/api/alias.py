...
alias_bt = Blueprint("alias", __name__, url_prefix='/aliases', description="alias check endpoints")


class AliasResponse(ma.Schema):
    used = ma.fields.Boolean(description="Indicator whether a username is already taken")
    date_used = ma.fields.DateTime(description="Indicates the date the username was taken, null if the username is not taken.",allow_none=True)
    username = ma.fields.String(description="The username being checked")
    owner = ma.fields.String(description="The name of the user who owns the username as on the platform.")
